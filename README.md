...
8. Open a long-term session exercise and use `Ctrl+J`; confirm a new dated entry is created for the current section only.
9. Open a long-term project exercise across a restart; confirm it reopens the same ongoing draft row for the active section instead of creating a new one.
10. Open `Ctrl+E`; confirm optional exercises are marked and archived exercises only appear after `Ctrl+A`.
11. Open `Ctrl+H`; confirm long-term history is scoped to the current section (`Freewrite` or `Exercise`) and entries open in read-only preview mode.
12. Open `Ctrl+T`; confirm the countdown replaces word count, autosave is suppressed mid-sprint, and completion saves plus rings the bell.
13. During autosave, confirm cursor position and editor scroll remain stable.
14. Restart the app; confirm mode, split ratio, and last exercise are restored from `~/.local/share/gbtw/progress.db`.
